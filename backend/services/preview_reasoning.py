...
**composition**:
   ...
   - visual_focus: ONLY when layout is "split" — the bounding box of the single best region of clean IMAGERY (products, scene, people, device), as fractions {"x","y","width","height"} 0-1 of the screenshot. It renders in a TALL PORTRAIT panel and cover-crops to fill. Hard rules for the box: (a) it must contain essentially NO readable words — not even partially at its edges; (b) many heroes have a big headline/tagline overlaid across the middle — do NOT crop through that text; pick a text-free zone such as a LEFT/RIGHT edge or a top/bottom strip of the hero, or better, a clean PRODUCT / category / gallery photo further down the page; (c) prefer a tall, portrait-ish region (taller than wide) so cover-crop barely trims it. If every part of the hero is covered by text, choose a product image lower on the page instead. null when layout is "typographic".
