...
**composition**:
   ...
   - visual_focus: ONLY when layout is "split" — the bounding box of the single best region of clean IMAGERY (products, scene, people, device), as fractions {"x","y","width","height"} 0-1 of the screenshot. It renders in a tall portrait panel and cover-crops to fill, so pick a region that still reads well when cropped. EXCLUDE the top nav bar, cookie/consent banners, text-only areas, AND — importantly — the site's own large overlaid HEADLINE / marketing text: that text duplicates our title and looks broken when sliced by the crop. Frame the actual pictures/products, not the words laid over them (e.g. pick a corner or side of the hero that is mostly image, or a product/gallery area lower on the page). Prefer a roughly square-to-portrait region. null when layout is "typographic".
