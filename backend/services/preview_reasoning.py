...
**composition**:
   ...
   - visual_focus: ONLY when layout is "split" — the box of the strongest BRAND imagery to feature. If the hero has a distinctive LOGO / MASCOT or a clear product, FEATURE it — that is strong identity and looks great in the panel. The one thing to avoid: do NOT let the box slice through the site's big overlaid HEADLINE / tagline (a line of large marketing words across the hero). The logo/mascot and product imagery usually sit ABOVE that headline, so frame the region around the logo + imagery and stop just before the headline line — capture the picture, not the sentence laid over it. A compact region centered on the subject (logo/product) works best; it renders in a tall portrait panel and cover-crops to fill. EXCLUDE nav bars and cookie banners. null when layout is "typographic".
