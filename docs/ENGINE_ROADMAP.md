...
**Status:** **implemented.** This document is kept as the record of *why* each change
was made and what the engine looked like before. For the engine as it now stands, read
[`ENGINE_ARCHITECTURE.md`](./ENGINE_ARCHITECTURE.md).

> Every phase below has shipped. The "what holds it back" table describes the engine
> as it was, not as it is — `preview_engine.py` is 407 lines rather than 4,100, there
> is one render stack rather than five, the multi-agent path is deleted, every fallback
> emits a reason code, and the corpus scores the rendered PNGs. Two things the roadmap
> did not anticipate are worth knowing about and are documented in the architecture
> note: scoring must be told when a card has no logo and which layout rendered, because
> both are design choices that a naive scorer reports as defects — and reports them
> loudly enough to replace a good card with a fallback.
